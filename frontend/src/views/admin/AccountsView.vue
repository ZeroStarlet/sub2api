        ...
        >
          ...
          <template #cell-platform_type="{ row }">
            <div class="flex flex-wrap items-center gap-1">
              ...
              <!-- Telemetry privacy stats indicator -->
              <span
                v-if="row.extra?.telemetry_privacy && row.extra?.telemetry_privacy_stats"
                class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-medium bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                :title="t('admin.accounts.telemetryPrivacyTooltip', { dropped: row.extra.telemetry_privacy_stats.dropped_telemetry ?? 0, stripped: (row.extra.telemetry_privacy_stats.stripped_body ?? 0) + (row.extra.telemetry_privacy_stats.stripped_headers ?? 0), log: row.extra.telemetry_privacy_stats.global_event_logging_drops ?? 0 })"
              >
                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                {{ t('admin.accounts.telemetryPrivacyStatus', { count: (row.extra.telemetry_privacy_stats.dropped_telemetry ?? 0) + (row.extra.telemetry_privacy_stats.stripped_body ?? 0) + (row.extra.telemetry_privacy_stats.stripped_headers ?? 0) }) }}
              </span>
            </div>
          </template>
